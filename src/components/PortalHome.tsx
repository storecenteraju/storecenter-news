...
export default function PortalHome({
  ...
  // Top 10 most viewed posts of the portal (ðŸ”¥ MAIS LIDAS DA SEMANA)
      ...
      <main className="max-w-7xl mx-auto px-4 py-4 md:py-10">
          ...
          <div className="bg-white border border-slate-200 p-6 rounded-lg mb-6 md:mb-8 shadow-sm">
            ...
            <h1 className="text-3xl font-extrabold font-display tracking-tight text-slate-950 uppercase border-b-2 border-primary pb-2 flex items-center justify-between">
              ...
              <span className="text-xs font-normal text-slate-400 uppercase lowercase">
                {finalFilteredPosts.length} {finalFilteredPosts.length === 1 ? 'notÃ­cia encontrada' : 'notÃ­cias encontradas'}
              </span>
            </h1>
            <p className="text-xs text-slate-500 mt-2 font-medium leading-relaxed">
              Cobertura jornalÃ­stica especializada do portal Store Center focada na cobertura do setor de <span className="lowercase font-bold text-slate-900">{selectedCategory}</span>.
            </p>
          </div>
        )}
        ...
        {finalFilteredPosts.length === 0 && (
          <div className="bg-white rounded-xl border border-slate-200 p-16 text-center shadow-sm max-w-2xl mx-auto my-6 md:my-12">
            ...
            <span className="font-bold text-slate-800 text-lg block mb-1">Nenhuma notÃ­cia disponÃ­vel</span>
        ...
        {finalFilteredPosts.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 md:gap-8">
              ...
              {selectedCategory === 'Home' && featurePost && (
                ...
                >
                  ...
                  <div className="md:w-2/5 p-6 md:p-8 flex flex-col justify-between">
                    <div>
                      {/* Selos acima do tÃ­tulo da matÃ©ria */}
                      <div className="flex flex-wrap gap-2 mb-3.5">
                        {isPostUrgente(featurePost) && (
                          <span className="bg-red-650 border border-red-700 text-white text-[10px] font-black uppercase tracking-wider px-3 py-1.5 rounded inline-flex items-center gap-1.5 select-none shadow-sm animate-pulse" title="NotÃ­cias quentes das Ãºltimas horas">
                            ðŸš¨ URGENTE <span className="text-[9.5px] font-medium lowercase font-sans opacity-90">(notÃ­cias quentes das Ãºltimas horas)</span>
                          </span>
                        )}
                        {(featurePost.views || 0) >= 500 && (
                          <span className="bg-red-750 border border-red-805 text-white text-[10px] font-black uppercase tracking-wider px-3 py-1.5 rounded inline-flex items-center gap-1.5 select-none shadow-sm" title="Mais de 500 visualizaÃ§Ãµes">
                            ðŸ”¥ MAIS LIDA <span className="text-[9.5px] font-medium lowercase font-sans opacity-95">(500+ visualizaÃ§Ãµes)</span>
                          </span>
                        )}
                        {top5DestaquesDaSemanaIds.has(featurePost.id) && (
                          <span className="bg-amber-500 border border-amber-600 text-slate-900 text-[10px] font-black uppercase tracking-wider px-3 py-1.5 rounded inline-flex items-center gap-1.5 select-none shadow-sm" title="SeleÃ§Ã£o editorial">
                            â­ DESTAQUE DA SEMANA <span className="text-[9.5px] font-semibold lowercase font-sans opacity-90">(seleÃ§Ã£o editorial)</span>
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between border-t border-slate-100 pt-4 mt-4">
                      <span className="text-[10px] font-bold text-primary hover:text-blue-700 uppercase tracking-wider flex items-center gap-1.5">
                        Ler MatÃ©ria Completa <ArrowRight className="w-3 h-3 transition-transform group-hover:translate-x-1" />
                      </span>
              ...
              <section className="space-y-6">
                <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                  <h3 className="text-base font-extrabold font-display text-slate-950 border-l-4 border-blue-600 pl-3 uppercase tracking-tight">
                    {selectedCategory === 'Home' ? 'Cobetura Recente' : `ColetÃ¢nea em ${selectedCategory}`}
                  </h3>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">AtualizaÃ§Ãµes em tempo real</span>
                </div>
                      ...
                      <div>
                        ...
                        <div className="p-5 flex flex-col">
                          ...
                          <div className="flex flex-wrap gap-1 mb-2.5">
                            {isPostUrgente(post) && (
                              <span className="bg-red-650 text-white text-[8.5px] font-black uppercase tracking-wider px-2 py-1 rounded inline-flex items-center gap-1 select-none shadow-sm animate-pulse" title="NotÃ­cias quentes das Ãºltimas horas">
                                ðŸš¨ URGENTE <span className="text-[7.5px] font-normal lowercase font-sans opacity-90 hidden sm:inline-block">(notÃ­cias quentes das Ãºltimas horas)</span>
                              </span>
                            )}
                            {(post.views || 0) >= 500 && (
                              <span className="bg-red-750 text-white text-[8.5px] font-black uppercase tracking-wider px-2 py-1 rounded inline-flex items-center gap-1 select-none shadow-sm" title="Mais de 500 visualizaÃ§Ãµes">
                                ðŸ”¥ MAIS LIDA <span className="text-[7.5px] font-normal lowercase font-sans opacity-95 hidden sm:inline-block">(500+ visualizaÃ§Ãµes)</span>
                              </span>
                            )}
                            {top5DestaquesDaSemanaIds.has(post.id) && (
                              <span className="bg-amber-500 text-slate-900 text-[8.5px] font-black uppercase tracking-wider px-2 py-1 rounded inline-flex items-center gap-1 select-none shadow-sm" title="SeleÃ§Ã£o editorial">
                                â­ DESTAQUE DA SEMANA <span className="text-[7.5px] font-medium lowercase font-sans opacity-90 hidden sm:inline-block">(seleÃ§Ã£o editorial)</span>
                              </span>
                            )}
                          </div>
                      ...
                      >
                        ...
                        <div>
                          <div className="flex flex-wrap items-center gap-1 mb-1">
                            ...
                            {isPostUrgente(post) && (
                              <span className="text-[8px] font-black text-red-600 uppercase flex items-center gap-0.5" title="ðŸš¨ URGENTE (notÃ­cias quentes das Ãºltimas horas)">
                                ðŸš¨ URGENTE <span className="text-[7px] font-normal lowercase font-sans opacity-80">(quentes)</span>
                              </span>
                            )}
                            {(post.views || 0) >= 500 && (
                              <span className="text-[8px] font-black text-red-700 uppercase flex items-center gap-0.5" title="ðŸ”¥ MAIS LIDA (500+ visualizaÃ§Ãµes)">
                                ðŸ”¥ MAIS LIDA <span className="text-[7px] font-normal lowercase font-sans opacity-80">(500+ views)</span>
                              </span>
                            )}
                            {top5DestaquesDaSemanaIds.has(post.id) && (
                              <span className="text-[8px] font-black text-amber-600 uppercase flex items-center gap-0.5" title="â­ DESTAQUE DA SEMANA (seleÃ§Ã£o editorial)">
                                â­ DESTAQUE <span className="text-[7px] font-normal lowercase font-sans opacity-80">(editorial)</span>
                              </span>
                            )}
                          </div>
            ...
            <div className="lg:col-span-1 space-y-6 md:space-y-8">
              ...
              {/* AS MAIS LIDAS INDEX (Sempre VisÃ­vel no Desktop/Mobile abaixo da Busca) */}
              ...
              <div className="bg-slate-900 p-5 rounded-xl border border-slate-800 text-slate-400 text-[11px] leading-relaxed space-y-2.5">
                ...
                <p>O Store Center Ã© um portal de notÃ­cias com conteÃºdo e informaÃ§Ã£o. Nosso objetivo Ã© oferecer informaÃ§Ãµes de qualidade sobre economia, negÃ³cios, tecnologia, polÃ­tica, investimentos e temas de interesse nacional, sempre com foco na clareza, agilidade e confiabilidade das informaÃ§Ãµes.</p>
                <p>Voltado para empreendedores, investidores, profissionais de tecnologia, estudantes e leitores em geral, o Store Center News busca tornar o acesso ao conhecimento mais simples, moderno e acessÃ­vel, combinando inovaÃ§Ã£o tecnolÃ³gica com conteÃºdo de valor para seus leitores.</p>
              </div>
        ...
        {/* BOTTOM ADSENSE BANNER CONTAINER - RODAPÃ‰ DESTAQUE */}
        {adFooter && (
      ...
      <footer className="bg-slate-900 text-slate-400 border-t border-slate-800 mt-10 md:mt-20 py-8 md:py-12">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-8 text-xs">
          <div>
            ...
            <p className="leading-relaxed">{siteSettings?.siteDescription || 'LÃ­der regional em jornalismo independente e cobertura setorial especializada.'}</p>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <span onClick={() => { onCategorySelect?.('PolÃ­tica'); }} className="hover:text-white transition-colors cursor-pointer">PolÃ­tica</span>
              <span onClick={() => { onCategorySelect?.('NegÃ³cios'); }} className="hover:text-white transition-colors cursor-pointer">NegÃ³cios</span>
              <span onClick={() => { onCategorySelect?.('GeopolÃ­tica'); }} className="hover:text-white transition-colors cursor-pointer">GeopolÃ­tica</span>
          ...
          <div>
            ...
            <p className="leading-relaxed mb-2">Suporte comercial, pautas corporativas e informaÃ§Ãµes de hospedagem:</p>
        ...
        <div className="max-w-7xl mx-auto px-4 mt-8 pt-8 border-t border-slate-800 text-center text-[11px] text-slate-500 flex flex-col md:flex-row items-center justify-between gap-4">
          <p>{siteSettings?.footerText || 'Â© 2026 Store Center Portal de News. Todos os direitos reservados. Design refinado e carregamento ultra-rÃ¡pido.'}</p>
          <button 
